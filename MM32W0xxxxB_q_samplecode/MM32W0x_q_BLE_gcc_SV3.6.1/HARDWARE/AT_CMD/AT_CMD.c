...
typedef void (*ATCMDFUNC)(
		u8 *cmd/*NULL ended, leading with the cmd NAME string, checking usage*/,
		u8 len);
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_SetName(u8 *parameter, u8 len) //AT+SETNAME=abcdefg
{
	if (len <= 8) {
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_SetAdvInt(u8 *parameter, u8 len) //AT+SETINTERVAL=160
{
	int v = 0;
	unsigned char i;
	if (len <= 12) {
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_SendData(u8 *parameter, u8 len) //AT+BLESEND=N,0x123456abc
{
	u8 *data = parameter;
	u8 data_len = 0, i;
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_SetAdvFlag(u8 *parameter, u8 len) //AT+SETADVFLAG=x   x=0/1, 0 disable adv, 1 enable adv
{
	if (len <= 11) {
		moduleOutData((u8*) "IND:ERR\n", 8);
		return;
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_DisconnecteBle(u8 *parameter, u8 len) //AT+DISCON, disconnect the connection is any
{
	if (len != 6) {
		moduleOutData((u8*) "IND:ERR\n", 8);
		return;
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_LowPower(u8 *parameter, u8 len) //AT+LOWPOWER=x   x=0/1/2
{
	if (len <= 9) {
		moduleOutData((u8*) "IND:ERR\n", 8);
		return;
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_SetBaud(u8 *parameter, u8 len) //AT+SETBAUD
{
	int v = 0;
	unsigned char i;
	if (len <= 8) {
...
//function list
AT_CMD_FUNC at_func_list[] = { { atcmd_SetName, "SETNAME=" }, { atcmd_SetAdvInt,
		"SETINTERVAL=" }, { atcmd_SendData, "BLESEND=" }, { atcmd_LowPower,
		"LOWPOWER=" }, { atcmd_SetBaud, "SETBAUD=" }, { atcmd_SetAdvFlag,
		"SETADVFLAG=" }, { atcmd_DisconnecteBle, "DISCON" }, { atcmd_Minfo,
		"MINFO" }, { atcmd_Help, "HELP" }, };
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_Minfo(u8 *parameter, u8 len) //AT+MINFO
{
	moduleOutData((u8*) "IND:OK\n", 7);
	moduleOutData((u8*) MM126_M305_VERSION_STRING,
			sizeof(MM126_M305_VERSION_STRING) - 1);
 ...
 **@return   None.
 ********************************************************************************************************/
void atcmd_Help(u8 *parameter, u8 len) //AT+HELP
{
	char i, templen;
	char name[20] = "AT+";
	moduleOutData((u8*) "IND:OK\n", 7);
...
void CheckAtCmdInfo(void) //main entrance
{
	while (comrxbuf_wr_pos != comrxbuf_rd_pos) //not empty
	{
		...
		if ((rxBuf[comrxbuf_rd_pos] == 0x0a)
				|| (rxBuf[comrxbuf_rd_pos] == 0x0d)) {
